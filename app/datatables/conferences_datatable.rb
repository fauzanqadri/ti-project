class ConferencesDatatable
	...
	def fetch_conferences
		...
		if current_user.userable_type == "Lecturer"
			...
			conferences = Conference.includes(:userable, skripsi: [{supervisors: :lecturer}, :student]).by_faculty(faculty.id).approved_supervisors.approved_department_director
		end
		...
		if params[:sSearch].present?
			...
			conferences = conferences.joins{skripsi.student}.where{(skripsi.title =~ "%#{query}%") | (students.full_name =~ "%#{query}%")}
		end
